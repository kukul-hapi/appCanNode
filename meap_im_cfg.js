...
adapterConfig.prototype.buildOption = function (name, query, context, cb) {
    ...
    LOG2("[meap_im_cfg][adapterConfig][buildOption]:", JSON.stringify(query));

    function buildpost(post) {
        LOG3("[meap_im_cfg][adapterConfig][buildOption][buildpost]:build post");
        var out = {};
        if (!post)
            return out;
        if (post.mutipart) {
    ...
    function buildsoapparams(params) {
        LOG3("[meap_im_cfg][adapterConfig][buildOption][buildsoapparams]:buildsoapparams");
        var out = {};
    ...
    function buildauth(auth) {
        LOG3("[meap_im_cfg][adapterConfig][buildOption][buildauth]:buildauth");
        var sec = null;
        LOG3("[meap_im_cfg][adapterConfig][buildOption][buildauth]:", JSON.stringify(auth));
        try {
            switch (auth.type) {
                case "wstext":
                case "wsdigest":
            ...
            _ERROR("[meap_im_cfg][adapterConfig][buildOption][buildauth][ERROR]:" + e.message);
            return null;
        }
    }
...
function parseCfg(path, cb) {
    ...
    try {
        ...
        _ERROR("[meap_im_cfg][parseCfg][ERROR]:" + e.message);
        config = new Config();
        config.init(path);
    }
    return config;
